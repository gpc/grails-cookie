...
class CookieHelperDefaultsSpec extends Specification implements GrailsWebUnitTest {
    ...
    @Unroll
    void "getDefaultCookieAge(): #defaultAge #maxAge #expectedMaxAge #comment"() {
        given:
        if (defaultAge != null) {
            grailsApplication.config.merge(['grails.plugins.cookie.cookieage.default': defaultAge])
        }
        expect:
        helper.getDefaultCookieAge(maxAge) == expectedMaxAge
        where:
        defaultAge | maxAge | expectedMaxAge | comment
    ...
    void "getDefaultCookiePath(): #defaultStrategy #path #expectedPath #comment"() {
        ...
        if (defaultStrategy != null) {
            grailsApplication.config.merge(['grails.plugins.cookie.path.defaultStrategy': defaultStrategy])
        }
        expect:
        helper.getDefaultCookiePath(path) == expectedPath
        where:
        ctx    | defaultStrategy | path    | expectedPath | comment
        '/ctx' | null            | null    | '/ctx'       | 'context strategy will be used if defaultStrategy not set'
        '/ctx' | 'context'       | null    | '/ctx'       | 'context option will be used if not set'
        null   | 'root'          | null    | '/'          | ''
    ...
    void "getDefaultCookieSecure(): #requestSecure #defaultSecure #secure #expectedSecure #comment"() {
        ...
        if (defaultSecure != null) {
            grailsApplication.config.merge(['grails.plugins.cookie.secure.default': defaultSecure])
        }
        expect:
    ...
    void "getDefaultCookieHttpOnly(): #defaultHttpOnly #httpOnly #expectedHttpOnly #comment"() {
        ...
        if (defaultHttpOnly != null) {
            grailsApplication.config.merge(['grails.plugins.cookie.httpOnly.default': defaultHttpOnly])
        }
        expect:
